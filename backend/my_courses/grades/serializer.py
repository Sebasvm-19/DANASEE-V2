...
class MateriaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Materia
        fields = ['id', 'nombre', 'nombre_display', 'descripcion', 'tematica', 'curso', 'profesor']
...
class NotaSerializer(serializers.ModelSerializer):
    nombre              = serializers.CharField(source='actividad.nombre', read_only=True)
...
class MateriaConNotasSerializer(serializers.ModelSerializer):
    ...
    nota_promedio_curso = serializers.SerializerMethodField()
    nota_periodo_estudiante = serializers.SerializerMethodField()

    class Meta:
        model = Materia
        fields = ['id', 'nombre', 'descripcion', 'tematica', 'notas', 'nota_promedio_curso', 'nota_periodo_estudiante']

    def get_notas(self, materia):
        estudiante = self.context.get('estudiante')
        notas = Nota.objects.filter(materia=materia, estudiante=estudiante)
        return NotaSerializer(notas, many=True).data

    def get_nota_promedio_curso(self, materia):
        # Promedio general ponderado de todos los estudiantes en esa materia
        notas = Nota.objects.filter(materia=materia)

        if not notas.exists():
            return None

        # Agrupar por estudiante y calcular nota ponderada
        estudiantes = {}
        for nota in notas:
            if nota.estudiante_id not in estudiantes:
                estudiantes[nota.estudiante_id] = []
            estudiantes[nota.estudiante_id].append((nota.valor, nota.actividad.porcentaje))

        promedios = []
        for notas_est in estudiantes.values():
            total = sum(valor * porcentaje / 100 for valor, porcentaje in notas_est)
            promedios.append(total)

        return round(sum(promedios) / len(promedios), 2) if promedios else None


    def get_nota_periodo_estudiante(self, materia):
        estudiante = self.context.get('estudiante')
        notas = Nota.objects.filter(materia=materia, estudiante=estudiante)

        periodos = {}
        for nota in notas:
            periodo = nota.actividad.periodo_academico
            periodos.setdefault(periodo, []).append((nota.valor, nota.actividad.porcentaje))

        return {
            periodo: round(sum(valor * porcentaje / 100 for valor, porcentaje in notas), 2)
            for periodo, notas in periodos.items()
        }
