              ...
              <Accordion.Body>
                ...
                {notas.length > 0 && (
                  <>
                    ...
                    {materia.nota_promedio_curso !== null && (
                      <p><strong>Promedio del curso en esta materia:</strong> {materia.nota_promedio_curso.toFixed(2)}</p>
                    )}

                    {materia.nota_periodo_estudiante && Object.keys(materia.nota_periodo_estudiante).length > 0 && (
                      <div>
                        <strong>Notas acumuladas por periodo:</strong>
                        <ul>
                          {Object.entries(materia.nota_periodo_estudiante).map(([periodo, nota]) => (
                            <li key={periodo}>Periodo {periodo}: {nota.toFixed(2)}</li>
                          ))}
                        </ul>
                      </div>
                    )}
