...
class MateriasDelEstudianteAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            raise NotFound("No se encontró un estudiante asociado a este usuario.")

        materias = Materia.objects.filter(curso=estudiante.grado)

        serializer = MateriaConNotasSerializer(
            materias,
            many=True,
            context={'request': request, 'estudiante': estudiante}
        )
...
class CursosDelProfesorAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            cursos = Curso.objects.filter(materia__profesor=profesor).distinct()
            return cursos
